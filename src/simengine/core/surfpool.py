...
class SurfpoolConfig:
    ...
    # Programs to deploy (path to .so files)
    programs_to_deploy: list[str] = None
    ...
    def __post_init__(self):
        ...
        self.programs_to_deploy = self.programs_to_deploy or []
        ...
        # If deploying programs, also disable offline mode
        if self.programs_to_deploy:
            self.offline_mode = False
...
class SurfpoolManager:
    ...
    def rpc_url(self) -> str:
        ...
        return f"http://127.0.0.1:{self.config.rpc_port}"
    ...
    def ws_url(self) -> str:
        ...
        if not self._started:
            raise RuntimeError("Surfpool not started")
        return f"ws://127.0.0.1:{self.config.ws_port}"
    ...
    def _build_command(self) -> list[str]:
        ...
        cmd.extend(["--ws-port", str(self.config.ws_port)])
        
        # Run in non-interactive mode for automation
        cmd.append("--no-tui")
        
        # Configure network/cloning strategy
        if self.config.clone_from:
            if self.config.clone_from in ["devnet", "mainnet", "testnet"]:
                # Lazy cloning mode - Surfpool fetches accounts on demand
                cmd.extend(["--network", self.config.clone_from])
            else:
                # Manual cloning from custom RPC
                cmd.extend(["--rpc-url", self.config.clone_from])
                
                # Check for explicit clones (legacy/manual mode)
                if self.config.programs_to_clone:
        ...
        # Deploy local programs (BPF programs)
        # Format: --bpf-program <PROGRAM_ID> <PROGRAM_SO_PATH>
        for program_path in self.config.programs_to_deploy:
            # Generate a deterministic program ID from the path
            import hashlib
            from base64 import b64encode
            h = hashlib.sha256(program_path.encode()).digest()
            program_id = b64encode(h).decode()[:44]
            cmd.extend(["--bpf-program", program_id, program_path])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    f"http://127.0.0.1:{self.config.rpc_port}",
                    json={
                        "jsonrpc": "2.0",
                        "id": 1,
                        "method": "getHealth",
